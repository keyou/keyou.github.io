...
因此，写了一个 Demo（demo_android_skia） 来模拟浏览器在 Android 上的渲染，用于加深对 Chromium 渲染的理解，也方便后续的调试。为了能够定量的分析程序的性能，在 Demo 中添加了 [Trace]({% post_url 2020-05-20-tracing %}) 机制，然后抓到性能数据如下：
...
Uptime: 259792491 Realtime: 543195951

** Graphics info for pid 16230 [org.demo.demo_android_skia] **

Stats since: 255753967478060ns
...
HISTOGRAM: 5ms=0 6ms=0 7ms=1 8ms=0 9ms=0 10ms=0 11ms=0 12ms=0 13ms=0
Font Cache (CPU):
  Size: 31.05 kB
  Glyph Count: 4
CPU Caches:
GPU Caches:
  Other:
    Other: 7.47 KB (1 entry)
    Texture: 9.00 KB (1 entry)
         Texture( 1):        9216 Byte (count:1)
  Scratch:
    Buffer Object: 48.00 KB (1 entry)
    Texture: 4.00 MB (1 entry)
         Texture( 1):     4194304 Byte (count:1)
Other Caches:
                         Current / Maximum
  VectorDrawableAtlas    0.00 kB /   0.00 KB (entries = 0)
  Layers Total           0.00 KB (numLayers = 0)
Total GPU memory usage (Max: 200 MB):
  4260320 bytes, 4.06 MB (9.00 KB is purgeable)

Pipeline=Skia (OpenGL)

Layout Cache Info:
  Usage: 17/5000 entries
  Hit ratio: 58/75 (0.773333)
Profile data in ms:

        org.demo.demo_android_skia/org.demo.demo_android_skia.DemoAndroidSkiaActivity/android.view.ViewRootImpl@69b5aec (visibility=0)
View hierarchy:

  org.demo.demo_android_skia/org.demo.demo_android_skia.DemoAndroidSkiaActivity/android.view.ViewRootImpl@69b5aec
  10 views, 14.13 kB of display lists

Total ViewRootImpl: 1
Total Views:        10
Total DisplayList:  14.13 kB

ProcessConfig(#0): {0.9 460mcc1mnc [zh_CN_#Hans] ldltr sw411dp w411dp h825dp 560dpi nrml long hdr widecg port finger -keyb/v/h -nav/h winConfig={ mBounds=Rect(0, 0 - 1440, 3040) mAppBounds=Rect(0, 149 - 1440, 3040) mWindowingMode=fullscreen mDisplayWindowingMode=fullscreen mActivityType=undefined mAlwaysOnTop=undefined mRotation=ROTATION_0} s.2961 desktop/d ?dc bts=0 ff=0 bf=0 themeSeq=0}
```
